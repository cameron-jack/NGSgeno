...
# Defines content of experiment.json which stores the current pipeline configuration
class Experiment():
    """
        ...
        with open(results_fn, 'rt') as rfn:
            for i, line in enumerate(rfn):
                ...
                if i == 0:
                    ...
                    if entry_id in entries_reported:
...
def load_experiment(exp_path):
    ...
    if os.path.exists(exp_file_path):
        with open(exp_file_path, 'rt') as f:
            expt_str = f.read()
            expt_str = expt_str.replace('src.experiment.Experiment', 'experiment.Experiment')
            # or the old location...
            expt_str = expt_str.replace('bin.experiment.Experiment', 'experiment.Experiment')
            expt = jsonpickle.decode(expt_str, keys=True, classes=Experiment)
            return expt
    else:
        return None