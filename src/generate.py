...
def run_generate(exp, target_func, *args, **kwargs):
    ...
    """
    transaction.clear_pending_transactions(exp)
    transaction.enforce_file_consistency(exp)
    success = target_func(*args, **kwargs)
    if not success:
        transaction.clear_pending_transactions(exp)
    else:
        if exp.pending_steps is not None and len(exp.pending_steps) > 0:
            clashes = transaction.clashing_pending_transactions(exp)
            if len(clashes) > 0:
                ...
                st.button('Accept', on_click=transaction.accept_pending_transactions, args=[exp], key='accept_overwrite_button')
                st.button('Cancel', on_click=transaction.clear_pending_transactions, args=[exp], key='cancel_overwrite_button')
            else:
...
def mk_picklist(exp, fn, rows, transactions, output_plate_guards=False, caller_id=None):
    ...
    Inputs:
        ...
        transactions - transaction dictionary of changes from the input plates
